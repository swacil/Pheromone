...
void CGui::displayTimer(CTimer timer)
{
	/*display calibration info*/
        float time = timer.getTime()/1000000.0; // getting time from timer
	char info[1000];
	SDL_Rect rect;				
	SDL_Surface *text;		
	int fontSize = 32;
	...
	rect.x = width-150;
	rect.y = fontSize;
	sprintf(info,"Elapsed time =  %.1f s", time);
	text = TTF_RenderUTF8_Blended(smallFont, info, ok_col);
	rect.x =  width-15 - text->w; 
